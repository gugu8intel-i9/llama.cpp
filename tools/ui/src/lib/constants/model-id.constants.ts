...
export const MODEL_ID = {
	 ...
	 * token; same lowercase-only rule as the prefix form.
	 */
	SIDECAR_INFIX_REGEX: new RegExp(`^(.*)-(${SIDECAR_TOKEN_ALTERNATION})-(.+)$`),
	 ...
	 *
	 * The token must be lowercase: the server's filename grammar (common/download.cpp)
	 * matches the same way, and a plain name carrying the token
	 * (`Model-MTP-BF16.gguf`) is a model, not a sidecar.
	 */
	SIDECAR_PREFIX_REGEX: new RegExp(`^(${SIDECAR_TOKEN_ALTERNATION})-(.*)$`),
	 ...
	 * quantized. Lowercase-only, like the prefix form.
	 */
	SIDECAR_SUFFIX_REGEX: new RegExp(`^(.*)-(${SIDECAR_TOKEN_ALTERNATION})(-draft)?$`),
