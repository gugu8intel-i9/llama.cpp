...
// a sidecar file carries its token as a lowercase name segment, e.g.
// `mmproj-Model-F16.gguf`, `Model-mtp-Q4_0.gguf` or `Model-Q4_0-mtp.gguf`;
// an uppercase segment (`Model-MTP-BF16.gguf`) is a plain model, as are
// unrelated names that merely contain the token (`smtp-Model.gguf`)
...
// name with the sidecar token segment removed, e.g. `Model-Q4_0-mtp` -> `Model-Q4_0`
static std::string strip_sidecar_token(const std::string & base, const std::string & token) {
    if (base == token) {
        return {};
    }
    if (base.rfind(token + "-", 0) == 0) {
        return base.substr(token.size() + 1);
    }
    const std::string seg = "-" + token + "-";
    if (auto pos = base.find(seg); pos != std::string::npos) {
        return base.substr(0, pos) + "-" + base.substr(pos + seg.size());
    }
    if (string_ends_with(base, "-" + token + "-draft")) {
        return base.substr(0, base.size() - token.size() - 7);
    }
    if (string_ends_with(base, "-" + token)) {
        return base.substr(0, base.size() - token.size() - 1);
    }
    return base;
}
...
static std::string sidecar_quant(const std::string & path, const std::string & token) {
    return get_gguf_split_info(strip_sidecar_token(stem_of(path), token) + ".gguf").tag;
}
...
static hf_cache::hf_file find_best_sibling(const hf_cache::hf_files & files,
    ...
    for (const auto & f : files) {
        ...
        auto split = get_gguf_split_info(strip_sidecar_token(stem_of(f.path), token) + ".gguf");
        auto bits   = quant_bits_from_tag(split.tag);
        auto diff   = std::abs(bits - model_bits);
        bool exact  = !tag_upper.empty() && split.tag == tag_upper;
...
common_download_hf_plan common_download_get_hf_plan(const common_params_model & model, const common_download_opts & opts) {
    ...
    if (!model.hf_file.empty()) {
        ...
        // the full tag is kept for embedded-draft files (e.g. `Model-Q4_0-mtp.gguf`),
        // so this only runs when no model file matched
...
std::vector<common_cached_model_info> common_list_cached_models() {
    ...
    for (const auto & f : files) {
        ...
        // cached `mtp-Model-Q4_0.gguf`, `Model-mtp-Q4_0.gguf` or
        // `Model-Q4_0-mtp.gguf` shows up as `<repo>:Q4_0-mtp`; a plain name
        // carrying the token (`Model-MTP-BF16.gguf`) stays a loadable model
        auto token = sidecar_token_of(f.path);
        auto split = token.empty()
            ? get_gguf_split_info(f.path)
            : get_gguf_split_info(strip_sidecar_token(stem_of(f.path), token) + ".gguf");

        if (split.index != 1 || split.tag.empty()) {
            continue;
        }

        std::string tag = split.tag;
        if (!token.empty()) {
            tag += "-" + token;
        }
